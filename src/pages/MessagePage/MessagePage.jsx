...
const MessagesPage = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            socket.off(authUser.uid);
        };
    }, [authUser, selectedUserBasic, messagesState]);
    ...
    const pressEnter = (event) => {
        if (event.keyCode === 13) {
            ...
            socket.emit("messages", { from: authUser.uid, to: selectedUserBasic.uid, messages: text })
            const tmp = { senderId: authUser.uid, text: text, isRead: false }
            const tmp2 = [...messagesState, tmp]
            setMessagesState(tmp2)
            setText('');
    ...
    return (
        <div>
            <Routes>
                <Route path="/new/" element={<SendMessageBox MessageCss={MessageCss} />} />
            </Routes>
            <div className={MessageCss.message_page_container}>
                <div className={MessageCss.message_page_main_area}>
                    <div className={MessageCss.message_page_left_area}>
                        <div className={MessageCss.message_page_left_area_upper}>
                            ...
                            <Link style={{ position: "absolute", right: "5%", top: "30%" }}
                                ...
                                <svg className={MessageCss._ab6_} color="rgb(38, 38, 38)"
                    ...
                    {messagesState.length ?
                        <div>
                            <MessageArea userName={selectedUserBasic.name} pp={selectedUserBasic.profilePicURL} >
                                {messagesState.map((m, index) => {
                                    if (selectedUserBasic.uid !== m.senderId) {
                            ...
                            <div className={MessageCss.message_page_message_area_messaging_area_bottom}>
                                <div className={MessageCss.message_page_message_area_message_box}>
                                    <svg aria-label="İfade Simgesi" className={MessageCss._ab6_} color="rgb(38, 38, 38)" fill="rgb(38, 38, 38)"
                                        height="24"
                                    ...
                                    <textarea style={{ height: "inherit", marginTop: "0.8%", maxHeight: "50%" }} value={text} onKeyDown={pressEnter} placeholder="Message..." className={MessageCss.message_box} onChange={event => { setText(event.target.value); }}>
                                    </textarea>
                                    {text.length !== 0 ?
                                        <div style={{ fontWeight: "bold", marginRight: "10px", color: "#0095F6" }}>Send</div>
                                        :
                                        <div style={{ display: "flex", flexDirection: "row" }}>
                                            <svg style={{ marginRight: "8px" }} aria-label="Add Photo or Video" className={MessageCss._ab6_} color="rgb(38, 38, 38)" fill="rgb(38, 38, 38)"
                                            ...
                                            <svg style={{ marginRight: "5px" }} aria-label="Like" className={MessageCss._ab6_} color="rgb(38, 38, 38)" fill="rgb(38, 38, 38)" height="24"
                                                role="img" viewBox="0 0 24 24" width="24">
                                                <path
                        ...
                        :
                        <div className={MessageCss.message_page_right_area}>
                            <svg className={MessageCss._ab6_} color="rgb(38, 38, 38)" fill="rgb(38, 38, 38)"
                            ...
                            <h3>Your Messages</h3>
                            <h5 className={MessageCss.all_gray}>Send private photos and messages to a friend or group.</h5>
                            <Link to="/direct/new"><div className={MessageCss.message_page_send_message_button}>
